        ...
        <nav className="flex items-center justify-between flex-wrap bg-white text-black border-b-2 p-4 lg:py-4 lg:px-56">
                    ...
                    <Menu>
                        <MenuButton as={Button} rightIcon={<BsChevronDown />}>
                            <div className='flex items-center my-2 rounded hover:cursor-pointer hover:bg-purpleD duration-300'>
                                <img src={currentUser?.profilePic !== "" ? currentUser?.profilePic : process.env.PUBLIC_URL + "avatar" + "4" + ".svg"} alt="" width="40" />
            ...
            <div className={`${isExpanded ? `block` : `hidden`
                ...
                <div className="text-sm lg:flex-grow lg:flex items-center">
                    ...
                    {currentUser == "" && <NavLink to="/login" className="block mt-8 mx-5 lg:inline-block lg:mt-0 mr-4"><button className='bg-gradient-to-r from-purpleD to-purpleL px-4 py-2 rounded-xl text-white text-lg hover:-translate-y-1 duration-300'>Login</button></NavLink>}
                </div>
            </div>
