...
const Comment = ({ comment, user, post, setPost }: any) => {
    ...
    return (
        <div>
            <LikesModal isOpen={isOpenLikes} onClose={onCloseLikes} likes={comment?.likes} />
            <Stack direction='row' h='auto' p={1} m={3} >
                <div className='flex items-center w-full justify-between'>
                    ...
                    <div className='flex items-center' >
                        {isLiked ? <AiFillHeart className='h-4 lg:w-5 lg:h-auto text-purpleL hover:cursor-pointer' onClick={() => likeComment(comment?._id)} />
                            :
                            <AiOutlineHeart className='h-4 lg:w-5 lg:h-auto text-purpleL hover:cursor-pointer' onClick={() => likeComment(comment?._id)} />}
